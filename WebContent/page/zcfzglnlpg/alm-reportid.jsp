...
<body>
	...
	<div class="easyui-panel"
		...
		<form id="serachFrom" method="post" style="margin-bottom: 0">
			<table cellpadding="5" border='0'
				...
				<tr>
					<td style="width: 8%; text-align: right;"><label>报送号:</label>
					</td>
					<td style="width: 13%;"><input id="reportId" name="reportId"
						class="easyui-textbox" data-options="prompt: '报送号'"
						style="width: 100%;"></td>
					<td style="width: 10%; text-align: right;"><label>报送年度:</label>
					</td>
					<td style="width: 13%;">
						<!-- <input id="year" name="year" class="easyui-textbox" data-options="prompt: '报送年度'"  style="width:100%;"> -->
						<input style="width: 100%;" name="year" class="easyui-textbox"
						data-options="  editable:true , disabled:false , prompt:'请输入年份' " />
					</td>
					<td style="width: 8%; text-align: right;"><label>报送季度:</label>
					</td>
					<td style="width: 13%;"><input name="quarter"
					...
					<td style="width: 8%; text-align: right;"><label>报送类型:</label>
					</td>
					<td style="width: 10%;"><input name="reportType"
						class="easyui-combobox" style="width: 100%;"
				...
				<tr>
					<td colspan="8" style="text-align: right; padding-right: 20px">
						...
						style="width: 8%;">查询</a> <a href="javascript:void(0)"
						class="easyui-linkbutton" onclick="reset()"
						data-options="iconCls:'icon-reload'" style="width: 8%;">重置</a>
					</td>
				</tr>
			</table>
	...
	<div class="easyui-panel" style="border: 0; width: 100%">
		<table id="dg" class="easyui-datagrid" toolbar="#toolbar"
			...
			pagination="true" striped="true" nowrap="false" autoRowHeight="false"
			style="table-layout: fixed; word-break: break-all; width: 100%; height: auto;"
			loadMsg="加载数据中...">
			<div id="toolbar">
				<a href="#" class="easyui-linkbutton" iconCls="icon-add"
					plain="true" onclick="newCalCfReportInfo()">新建</a> <a href="#"
					class="easyui-linkbutton" iconCls="icon-cancel" plain="true"
					onclick="destroyCalCfReportInfo()">删除</a><a href="#" class="easyui-linkbutton" iconCls="icon-remove" plain="true" onclick="send()">需求发送</a>    	
			...
			<thead>
				<tr>
					...
					<th field="reportTypeName" width="15%" align="center"
						halign="center">报送类型</th>
					<th field="reportCateGory" width="12%" hidden>报告类型</th>
					<th field="reportCateGoryName" width="15%" align="center"
						halign="center">报告类型</th>
	...
	<div id="dlg" class="easyui-dialog"
		...
		<form id="fm" method="post">
			<table cellpadding="5">
				<tr>
					<td style="width: 20%;"><label>报送年度:</label></td>
					<td style="width: 30%;"><input id="year"
						class="easyui-textbox" name="year"
						data-options="prompt: '报送年度',required:true,editable:false,"
						style="width: 60%;"></td>
				</tr>
				<tr>
					<td style="width: 20%;"><label>报送季度:</label></td>
					<td style="width: 30%;"><input id="quarter" type="text"
						name="quarter" class="easyui-combobox" editable="false" required
						style="width: 60%;"
						data-options="
					        prompt: '报送季度',
				...
				<tr>
					<td style="width: 20%;"><label>报送类型:</label></td>
					<td style="width: 30%;"><input id="reportType" type="text"
						name="reportType" class="easyui-combobox" editable="false"
						required style="width: 60%;"<%-- data-options="
					        prompt: '报表类别',
				...
				<tr id='needHidden'>
					<td style="width: 20%;"><label>报告类型:</label></td>
					<td style="width: 30%;"><input id="reportCateGory" type="text"
						name="reportCateGory" class="easyui-combobox" editable="false"
						style="width: 60%;"
						data-options="
					        prompt: '报告类别',
				...
				<tr>
					<td style="width: 20%;"><label>报送日期:</label></td>
					<td style="width: 30%;"><input type="text" name="dataDate"
						class="easyui-datebox" editable="false" required
						style="width: 60%;"></td>
				</tr>
			</table>
		</form>
		<div id="dlg-buttons">
			<a href="#" class="easyui-linkbutton" iconCls="icon-ok"
				onclick="saveCalCfReportInfo()">保存</a> <a href="#"
...
$(function() {
	...
	function newCalCfReportInfo() {
		...
		$('#fm').form('clear');
		$('#needHidden').hide();
		url = '${pageContext.request.contextPath}/alm_itemdefine/add.do';
				...
				.combobox(
						{
							...
							onLoadSuccess : function(data) {
								//alert(data[1].value);
								$("#reportType").combobox("setValue", '2');
								//$(this).combobox("select", data[1].value);
								$('#needHidden').show();
								$('#reportCateGory').combobox('setValue', '1');
								$('#reportCateGory').combobox('disable');
							}
						});
	}
	...
	function editCalCfReportInfo() {
		...
		$('#fm').form('load', rows[0]);
		url = '${pageContext.request.contextPath}/calCfReportInfo/update.do?id='
				+ rows[0].reportId;
				...
				.confirm(
						...
						function(r) {
							if (r) {
								$
										.post(
												'${pageContext.request.contextPath}/calCfReportInfo/delete.do',
												data,
												function(result) {
													if (result.success) {
														$('#dg').datagrid(
	...
	function edit() {
		...
		if (row) {
			...
			$('#fm').form('load', row);
			var array = row.tableCode + '&&colCode=' + row.colCode
					+ '&&yearMonth=' + row.yearMonth + '&&quarter='
					+ row.quarter + '&&reportRate=' + row.reportRate
			url = '${pageContext.request.contextPath}/RowAdd/update.do?tableCode='
					+ array;
		}
	}
	//保存
	...
	function exportXls(gridId, xlsName) {
		...
		jsutil.core.download(
				"${pageContext.request.contextPath}/indexCode/download", param);
	}
	...
	function send() {
		...
		var data = [ {
			name : 'reportId',
			value : rows[0].reportId
		} ];
		if (rows.length == 0) {
			$.messager.alert('消息提示', '请选择要需求发送的单号!', 'error');
			return;
		}
		if (rows.length > 1) {
			$.messager.alert('消息提示', '请选择一个要需求发送的单号!', 'error');
			return;
		}
		$.messager
				.confirm(
						...
						function(r) {
							if (r) {
								$
										.post(
												'${pageContext.request.contextPath}/alm_itemdefine/send.do',
												data,
												function(result) {
													if (result.success) {
														$('#dg').datagrid(
