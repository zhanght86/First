...
<%@include file="/commons/statics.jsp"%>
	...
	<!-- </div>	<div id="ft" style="text-align:center;width:auto;border-top: 0;background: #fff;">
...
<!-- 	操作，展示部分 -->
	 <div  class="easyui-panel" style="height: auto; width:100%; overflow: hidden;">
		...
		data-options="
			...
			>
			<div id="toolbar">
				<a href="#" class="easyui-linkbutton" iconCls="icon-add" plain="true" onclick="newCalPreCompute()">初步计算</a> 
				<a href="#" class="easyui-linkbutton" iconCls="icon-print" plain="true" onclick="modeldownloaddate()">数据下载</a> 
				<a href="#" class="easyui-linkbutton" iconCls="icon-cancel" plain="true" onclick="destroyCalPreCompute()">撤销初步计算</a>
			</div>
			<thead>
				<tr>
...
// 新建
   function newCalPreCompute(){
	...
	$('#dlg').dialog('open').dialog('setTitle','初步计算');
	$('#msg').text('正在进行初步计算，请稍后.');
	var data =[{name:'reportId',value:rows[0].reportId },{name:'reporttype',value:rows[0].reportCategory}];
	$.post('${pageContext.request.contextPath}/calPreCompute/precompute.do',data,function(result){
		if (result.success){
			$('#dlg').dialog('close');		// close the dialog
			$('#dg').datagrid('reload');	// reload the user data
			endEditing();
			jsutil.msg.alert("初步计算完成");
		} else {
			$('#dlg').dialog('close');		// close the dialog
			$('#dg').datagrid('reload');	// reload the user data
			endEditing();
