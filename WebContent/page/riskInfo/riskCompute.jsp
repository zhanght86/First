...
function riskCompute(){
	...
	$('#msg').text('正在进行风险指标计算，请稍后.');
	var data =[{name:'reportId',value:rows[0].reportId }];
	$.post('${pageContext.request.contextPath}/riskCompute/complexcompute.do',data,function(result){
		if (result.success){
			$('#dlg').dialog('close');		// close the dialog
			$('#dg').datagrid('reload');	// reload the user data
			endEditing();
			jsutil.msg.alert("风险指标计算完成");
		} else {
			$('#dlg').dialog('close');		// close the dialog
			$('#dg').datagrid('reload');	// reload the user data
			endEditing();
...
function destroyRiskCompute(){
	...
	$.messager.confirm('撤销操作','确认撤销吗?',function(r){
		if (r){
			$.post('${pageContext.request.contextPath}/riskCompute/backprecompute.do',data,function(result){
				if (result.success){
					$('#dg').datagrid('reload');	// reload the user data
