...
//      simpleAuthorInfo.addStringPermissions(permissionList);  
        ...
        if(null!=usercode && "lisi".equals(usercode)){  
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authcToken) throws AuthenticationException {  
		...
		List<?> l=userInfoDao.getByPropName(UserInfo.class, "userCode", token.getUsername());
        UserInfo u=(UserInfo) l.get(0);
        if(null != u){  
        SimpleAuthenticationInfo authcInfo = new SimpleAuthenticationInfo(u, u.getPassword(), this.getName());  
        if (!passwordEncryptService.saltDisabled()) {
        	authcInfo.setCredentialsSalt(ByteSource.Util.bytes(u.getSalt() + u.getUserCode()));
		}
