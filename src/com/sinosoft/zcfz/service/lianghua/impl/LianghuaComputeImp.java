...
@Service
public class LianghuaComputeImp implements LianghuaComputeService{
	...
	private static final Logger LOGGER = LoggerFactory.getLogger(LianghuaComputeImp.class);
	...
	public void ComplexCompute(String id,UserInfo user) throws Exception{
		...
		String lsql2="delete from ALM_CFREPORTDATA where month="+year+" and quarter="+quarter+" and outitemcode in (select portitemcode from cal_riskrelation where portitemtype='0' and interfacetype='1')";
		try{
			dao.excute(lsql2);
		}catch(Exception e){
		...
		for(int c=1;c<=ifpc;c++){
			...
			List<CalRiskRelation> list=(List<CalRiskRelation>) dao.query("from alm_RiskRelation where interfaceType='1' and interfacePriority='"+interfacePriority+"'");
			for(int i=0;i<list.size();i++){
				...
				if(interfaceGetData!=null&&!"".equals(interfaceGetData)){
					if(interfaceGetData.contains("select")){
						...
						listData=(List<Map>) dao.getList("select case when outitemtype in ('01','07') then  textvalue else to_char(reportitemvalue) end as value from ALM_CFREPORTDATA where month=? and quarter=? and outitemcode=?",year,quarter,interfaceGetData);
	...
	public void backComplexCompute(String id) {
		CalCfReportInfo ccri=new CalCfReportInfo();
		ccri=(CalCfReportInfo) dao.get(CalCfReportInfo.class, id);
		String sql="delete from ALM_CFREPORTDATA where reportid='"+id+"' and source='9'";
		String sql1="delete from ALM_CFREPORTROWADDDATA where reportid='"+id+"' and source='9'";
		try{
			dao.excute(sql);
			dao.excute(sql1);
		}catch(Exception e){
