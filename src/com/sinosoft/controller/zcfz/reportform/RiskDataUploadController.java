...
@Controller
@RequestMapping(path="/riskdataupload")
public class RiskDataUploadController {
	private static final String CalCfReportInfo = null;
	@Resource
	private UploaderServlet us;
	@Resource
	private RiskUploadService riskus;
	@Resource
	...
	public synchronized Map<String, Object> upload(UploadInfoDTO pld,MultipartFile file,HttpServletRequest request,HttpServletResponse response) throws Exception{
		...
		try {
			...
			pld.setFilePath(Config.getProperty("uploaddir")+"/RiskDataUpload");//文件上传地址
	...
	public InvokeResult modeDownload(UploadInfoDTO pld,HttpServletRequest request,HttpServletResponse response){
		...
		*/
			...
			u.setFilePath(split[0].toString()
			    ...
			    u.setFileName("RiskRatingMode.xlsx");
			...
			/*if("yes".equals(deptflag)){
				...
				}else if("2".equals(param)){//分支机构模板下载(分部门)
					newFileName = newFileName + "branch_" + dept + "_" + u.getFileName();
				}else if("3".equals(param)){//再保险机构模板下载(分部门)
					newFileName = newFileName + "R_" + dept + "_" + u.getFileName();
				}else if("1-data".equals(param)){//法人机构数据模板下载(分部门)
				...
				System.out.println("no----"+param+"no-----"+deptflag);
				if("all".equals(param)){//完整模板下载
				    ...
				    downloadPath =Config.getProperty("uploaddir")	+ "/RiskDataUpload/"+newFileName;
				...
				/*else if("1".equals(param)){//法人机构模板下载
					...
					newFileName = newFileName + "R_" + u.getFileName();
			...
			/*String newFilePath = Config.getProperty("uploaddir") + Config.getProperty("downloaddir");
